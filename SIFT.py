...
class SIFT_:
	...
	def get_features(self):
		...
		self.Extrema()
		#self.Plot_features()
		self.extrema = self.Accurate_keypoint_localozation_filter()
		self.extrema = self.Hessian()
	...
	def Extrema(self):
		...
		for octave in range(self.octaves):
			#scale_factor = 2**(-1+octave)
			self.extrema.append([])
			row,col = self.dog[octave][0].shape
			for level in range(2,2+self.scales):
			#	plt.figure()
				...
				for x in range(1,row-1):
					for y in range(1,col-1):
						if np.argmax(self.dog[octave][level-1:level+2,x-1:x+2,y-1:y+2]) == 13:
							self.extrema[octave][level-2].append((x,y,self.prior*(self.factor**(level))))
			#				plt.plot(y*scale_factor, x*scale_factor,marker='o', markerfacecolor='none',markeredgecolor='r')
				#np.save('octave'+str(octave)+'_'+str(level-1)+'extrema.npy',self.extrema[octave][level-1])
				print('should plot')
			#	plt.show()
	...
	def Plot_features(self):
		...
		for i in range(self.octaves):
			for j in range(self.scales):
				for x,y,s in self.extrema[i][j]:
					plt.plot(x*(2**(-1+i)), y*(2**(-1+i)),marker='o', markerfacecolor='red')
		plt.show()
		
	def Accurate_keypoint_localozation_filter(self):
		print('Processing Accurate_keypoint_localozation_filter')
		extrema_filtered = []
		for i in range(self.octaves):
			scale_factor = 2**(-1+i)
			...
			for j in range(self.scales):
				#plt.figure()
				#plt.imshow(self.origin_img,plt.get_cmap('gray'))
				ratio = 0
				extrema_filtered[i].append([])
				for x,y,s in self.extrema[i][j]:
					D3_cude = [self.dog[i][j+3][x-1:x+2,y-1:y+2],self.dog[i][j+2][x-1:x+2,y-1:y+2],self.dog[i][j+1][x-1:x+2,y-1:y+2]]
					keep = self.derivatives_filter(D3_cude)
					if keep:
						ratio += 1
						extrema_filtered[i][j].append((x,y,s))
				#		plt.plot(y*scale_factor, x*scale_factor,marker='o', markerfacecolor='none',markeredgecolor='r')
				print('portion of keypoints keep:',ratio/(self.scales*len(self.extrema[i][j])))
				#plt.show()
	...
	def derivatives_filter(self,data): #data will be 3*3*3
	...
	def Hessian(self):
		print("computing Hessian to reject low contrast keypoints")
		threshold = (self.ratio + 1) ** 2 / self.ratio
		...
		for i in range(self.octaves):
			...
			for j in range(self.scales):
				...
				for y,x,s in self.extrema[i][j]:
					...
					if check <= threshold:
						ratio += 1
						extrema_filtered[i][j].append((y,x,s))
						#self.extrema[i][j][k] = self.extrema[i][j][k] + (1,)
						#plt.plot(x*scale_factor, y*scale_factor,marker='o', markerfacecolor='none',markeredgecolor='r')
					#else:
					#	self.extrema[i][j][k] = self.extrema[i][j][k] + (0,)
				print('portion of keypoints keep:',ratio/(self.scales*len(self.extrema[i][j])))
				plt.show()
		return extrema_filtered
...
def get_descriptor(m,o,o_cent):							#16 by 16 block
	sig = int(16/2)
	gau = kernel(sig)
	cent = 2*sig
	...
	for i in range(4):
		descriptor.append([])
		for j in range(4):
			descriptor[i].append(get_4by4_patch(m_[i*4:(i+1)*4,j*4:(j+1)*4],o[i*4:(i+1)*4,j*4:(j+1)*4],o_cent))
	#visualize_descriptor(descriptor)
	return (descriptor,)

def get_4by4_patch(m,o,cent):						#4 by 4 patch orientation and magnitude
	des_ = [0,0,0,0,0,0,0,0]
	d_bar = np.zeros(o.shape)
	for i in range(4):
		for j in range(4):
			d_bar[i,j] = 1 - abs(o[i,j]-cent)/np.pi if abs(o[i,j]-cent) <=np.pi else 1 - abs(o[i,j]-cent-np.pi)/np.pi
	weighted_m = m*d_bar
...
def visualize_descriptor(des):
	...
	for i, ax in enumerate(fig.axes):
		...
		for ind,d in enumerate(des_):
				ax.arrow(0, 0, d*math.cos(ind*np.pi/4), d*math.sin(ind*np.pi/4), head_width=0.005, fc='k', ec='k')
		ax.set_aspect('equal')
	plt.show()
